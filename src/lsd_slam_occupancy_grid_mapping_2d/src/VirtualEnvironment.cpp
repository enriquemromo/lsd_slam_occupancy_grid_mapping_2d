...
{
	return ((x*(Xmax-Xmin))/screenSize) + Xmin;
...
{
	return ((y*(Ymax-Ymin))/(-1*screenSize)) + Ymax;

}


void VirtualEnvironment::generate(std::vector<Point> voxels)
{
	...
	std::stringstream ss;
	int index = 0; 
	double sizeZVoxel = (maxY - (minY)); //Tamaño en Z
	double z = ((maxY - (minY))/2); // Posición en Z
	...
	for(it = voxels.begin(); it != voxels.end(); ++it )
	{
		...
		world << "      <pose> "<<changeX((*it).x)<<  " " <<  changeY((*it).y) <<" "
		<< z <<" 0 -0 0</pose>" << endl;
		world << "      <link name='link'>" << endl;
		world << "        <inertial>" << endl;
		...
		world << "          <geometry>" << endl;
		world << "            <box>" << endl;
		world << "              <size>"<< voxelSize*2 <<  " " << voxelSize*2 <<" "
		<< sizeZVoxel <<"</size>" << endl;
		world << "            </box>" << endl;
		world << "          </geometry>" << endl;
		world << "          <max_contacts>10</max_contacts>" << endl;
		//world << "          <surface>" << endl;
